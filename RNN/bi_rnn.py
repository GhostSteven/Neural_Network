...
fcell = tf.nn.rnn_cell.LSTMCell(num_hidden)
bcell = tf.nn.rnn_cell.GRUCell(num_hidden)
...
def dynamic_bi_rnn(fcell,bcell,x,w,b):
	outputs, states1, states2 = tf.nn.bidirectional_dynamic_rnn(fcell, bcell, x, dtype = tf.float32)
	outputs = tf.transpose(tf.concat(outputs,2),[1,0,2])
	return tf.matmul(outputs[-1],w)+b


def static_bi_rnn(fcell,bcell,x,w,b):
	x = tf.unstack(x,timesteps,1)
	outputs, states1, states2 = tf.nn.static_bidirectional_rnn(fcell, bcell, x, dtype = tf.float32)
	return tf.matmul(outputs[-1],w)+b

if Dynamic_mode == True:
	logits = dynamic_bi_rnn(fcell,bcell,X,W,b)
else:
	logits = static_bi_rnn(fcell,bcell,X,W,b)
