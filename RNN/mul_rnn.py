...
cell1 = tf.nn.rnn_cell.LSTMCell(num_hidden)
cell2 = tf.nn.rnn_cell.GRUCell(num_hidden)
cell3 = tf.nn.rnn_cell.LSTMCell(num_hidden)

mcell = tf.nn.rnn_cell.MultiRNNCell([cell1,cell2,cell3])
...
def dynamic_mul_rnn(mcell,x,w,b):
	outputs, states = tf.nn.dynamic_rnn(mcell, x, dtype = tf.float32)
	outputs = tf.transpose(tf.concat(outputs,2),[1,0,2])
	return tf.matmul(outputs[-1],w)+b


def static_mul_rnn(mcell,x,w,b):
	x = tf.unstack(x,timesteps,1)
	outputs, states = tf.nn.static_rnn(mcell,x, dtype = tf.float32)
	return tf.matmul(outputs[-1],w)+b

if Dynamic_mode == True:
	logits = dynamic_mul_rnn(mcellX,W,b)
else:
	logits = static_mul_rnn(mcell,X,W,b)
